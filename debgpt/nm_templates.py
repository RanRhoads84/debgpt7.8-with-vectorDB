...
NM_TEMPLATES = {
        ...
        'pp1e.PH0': '''First, please explain the key points of the Social Contract and the DFSG _in your own words_. Also, describe what you think about these documents. Please don't just rephrase each point of the Social Contract and the DFSG, but try to summarize it as a whole.''',
        'pp1e.PH8': '''Donald Knuth, author of TeX, insists that no-one has the right to modify the source code of TeX, and that any changes must be made using "change files" (a sort of patch file). Is this allowed for a program for the main component of Debian?''',
        'pp1e.PH9': '''The GNU Free Documentation License (GFDL) has been heavily discussed on debian-legal in the past, leading to a GR [6].  Please summarize the problems Debian has with the GFDL, and what you think about the GR outcome.  [6] https://www.debian.org/vote/2006/vote_001''',
        'pp1e.PHb': '''An upstream released the first version of his program under GPL-2+ license. A year later he decided to release the version 2.0 of the same program under BSD-3-Clause. Is there any licensing violation here? Can this software be packaged in Debian?''',
}
