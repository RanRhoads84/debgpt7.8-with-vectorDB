...
Query Composer is a function that reads the plain text contents from the
specified resource, and wrap them as a part of a prompt for the LLM. In the
previous section we have seens the special query composer `MapReduce`, which
works differently from the standard query composers that will be introduced
here.
